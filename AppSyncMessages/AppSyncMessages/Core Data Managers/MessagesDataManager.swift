...
class MessagesDataManager: CoreDataManager {
    ...
    override init(){
        ...
        if let luiz = ProfileDataManager.shared.createProfile(name: "Luiz Mota", id: 11111, image: UIImage(named: "LoginBackground1")!, isOnline: true) {
            createMessage(text: "We should be doing Unit Tests...", profile: luiz, date: Date(), isSender: false)
        }
        ...
        if let chris = ProfileDataManager.shared.createProfile(name: "Chris Schumann", id: 22222, image: UIImage(named: "LoginBackground2")!, isOnline: false) {
            createMessage(text: "I can't, I have kids!!!", profile: chris, date: Date(timeInterval: -60, since: Date()), isSender: false)
        }
        ...
        if let brett = ProfileDataManager.shared.createProfile(name: "Brett Schumann", id: 33333, image: UIImage(named: "LoginBackground3")!, isOnline: false) {
            createMessage(text: "Living is winning!", profile: brett, date: Date(timeInterval: -120, since: Date()), isSender: false)
        }
        ...
        if let rubens = ProfileDataManager.shared.createProfile(name: "Rubens Neto", id: 44444, image: UIImage(named: "LoginBackground4")!, isOnline: false) {
            createMessage(text: "Pub?", profile: rubens, date: Date(timeInterval: -180, since: Date()), isSender: false)
        }
        ...
        if let tibo = ProfileDataManager.shared.createProfile(name: "TiBo", id: 55555, image: UIImage(named: "LoginBackground5")!, isOnline: true) {
            createMessage(text: "Do you have any pain killers?", profile: tibo, date: Date(timeInterval: -240, since: Date()), isSender: false)
            createMessage(text: "Hey TiBo! Don't worry man, everything is gonna be alright, how can I help you?", profile: tibo, date: Date(timeInterval: -270, since: Date()), isSender: true)
            createMessage(text: "I'm totally in pain for more than a month now and this fucking sucks!!! I'm not kidding, I'm gonna kill someone.", profile: tibo, date: Date(timeInterval: -300, since: Date()), isSender: false) 
        }
    }
    ...
    func createMessage(text: String, profile: Profile, date: Date, isSender: Bool) {
        var id: String!
